...
function loadModifiers() {
  const defaults = { title: 0, constructionAid: 0, vip: 0, lifetimePass: false, monthlyPass: false, builderClassPct: 0, masterBuilderPct: 0 };
...
// The Tech calculator (same origin) stores current tech levels under this key — read it here to
// auto-detect the Development bonus from Precision Engineering / Efficient Construction I-IV
// instead of a manual "Development maxed" checkbox.
const TECH_LS_KEY = 'palmon_current_levels_v1';
const DEV_PRECISION_ENGINEERING_KEY = 'tech_name_new_100900_9_title';
const DEV_EFFICIENT_CONSTRUCTION_KEYS = ['tech_name_new_100100_1_title', 'tech_name_new_101600_16_title', 'tech_name_new_102300_23_title', 'tech_name_new_103000_30_title'];
function loadTechLevels() {
  try { return JSON.parse(localStorage.getItem(TECH_LS_KEY) || '{}'); }
  catch(e) { return {}; }
}
...
// Gold/Lumber/Steel/Electricity are the only resources ever discounted — pink materials
// (Mastery Sigil, Totem Essence, Pharaoh Coin, etc.) are never reduced by any modifier.
const isReducible = (k) => k.startsWith('Gold') || k.startsWith('Lumber') || k.startsWith('Steel') || k.startsWith('Electricity');
function factorForResource(k, resFactor) { return isReducible(k) ? resFactor : 1; }

function fmtAdjustedResource(k, raw, resFactor) {
  const factor = factorForResource(k, resFactor);
...
function renderLevelBreakdown(building, currentLevel, resFactor, timeFactor) {
  ...
  return upcoming.map(lvl => {
    const resLine = Object.entries(lvl.cost || {}).map(([k, v]) => {
      const adj = v * factorForResource(k, resFactor);
...
function recalcAll() {
  ...
  // Development's bonus is auto-detected from the Tech calculator's saved levels for Precision
  // Engineering (-0.5% resource cost per level, max 2.5% at level 5) and Efficient Construction I-IV
  // (+1% Building Speed per level each, max 20% total once all 4 are at level 5) instead of a manual
  // "Development maxed" checkbox.
  const techLevels = loadTechLevels();
  const devResourcePct = (techLevels[DEV_PRECISION_ENGINEERING_KEY] || 0) * 0.5;
  const devSpeedPct = DEV_EFFICIENT_CONSTRUCTION_KEYS.reduce((sum, k) => sum + (techLevels[k] || 0), 0) * 1;
  const resourceDiscountPct = devResourcePct + (mod.builderClassPct || 0);
  ...
  const speedSources = [devSpeedPct, mod.title || 0, mod.constructionAid || 0, mod.vip || 0,
  ...
  const infoEl = document.getElementById('dev-computed');
  if (infoEl) {
    infoEl.textContent = `Auto-detected (from Tech calculator levels): Development bonus \u2192 -${devResourcePct.toFixed(1)}% Resource Cost, +${devSpeedPct.toFixed(1)}% Building Speed`;
  }
...
def main():
    ...
    parts.append('<div id="dev-computed" class="note" style="margin-bottom:10px;"></div>')
    ...
    parts.append('<label>Master Builder: <select id="mod-master-builder" class="mod-input">'
                 ...
                 '</select></label>')
    parts.append('<div class="note">Building-speed sources (Development, Title, Construction Aid, VIP, Lifetime Pass, Monthly Pass) compound sequentially/multiplicatively into a single time factor (verified against real in-game timers). Master Builder reduces build time directly (up to -25%) on top of that, it does NOT discount resource cost. Development\'s bonus (Precision Engineering -0.5%/level Resource Cost + Efficient Construction I-IV +1%/level Building Speed each) is auto-detected from the levels you entered in the Tech calculator (same browser, saved automatically there) — no manual checkbox needed; open the Tech calculator once and enter those levels if the computed value above reads 0%. Development and Builder Class Buff discount resource cost and stack additively. Note: Desert-tech research that reduces Desert-building time/resource cost is not yet factored into this calculator.</div>')
    parts.append('</div>')
